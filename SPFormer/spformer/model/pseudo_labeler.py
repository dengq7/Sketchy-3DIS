...
class PseudoLabeler(nn.Module):
    ...
    def get_ps_instance3D(self, ps_instance_label, ps_semantic_label, superpoint, scan_id, coord_float):
        #num_insts = ps_instance_label.max().item() + 1
        
        if ps_instance_label == None:
            gt_inst = torch.zeros(0, dtype=torch.int64)
            print('Pseudo instance_box of someone scene is none.')
            inst = Instances3D(0, gt_instances=gt_inst.numpy())
        ...
        num_insts = torch.unique(ps_instance_label)
        num_points = len(ps_instance_label)
        gt_masks, gt_labels, gt_boxes = [], [], []

        gt_inst = torch.zeros(num_points, dtype=torch.int64)

        for i in num_insts:
            idx = torch.where(ps_instance_label == i)
            
            assert len(torch.unique(ps_semantic_label[idx])) == 1
            sem_id = ps_semantic_label[idx][0].long()
            if sem_id == 18:
                # sem_id = 1
        ...
        if gt_masks:
            gt_masks = torch.stack(gt_masks, dim=0).cuda()
            superpoint_t = torch.sub(superpoint, superpoint.min().item())
            #print('superpoint_t:', superpoint_t.shape)
            #print('superpoint_t:', superpoint_t)
            gt_spmasks = torch_scatter.scatter_mean(gt_masks.float(), superpoint_t, dim=-1)
            #print('gt_spmasks.shape:', gt_spmasks.shape)
    ...
    def fit_gp_spp(self, coords_float_spp, feats_spp, b1_inds, b2_inds, intersect_inds, training_iter=50):
    ...
    def filter_bg_within_box(self, coords_float_spp, feats_spp, b_inds, box):
        ...
        uncertain_inds = torch.where(dis < threshold_coord)
    ...
    def filter_bg_within_box(self, coords_float_spp, feats_spp, b_inds, box):
        ...
        uncertain_inds = torch.where(dis < threshold_feat)
        pred_probs_new = dis[uncertain_inds]
        filter_inds = b_inds[uncertain_inds]
    ...
    def forward(self, scan_id, coord_float, superpoint, point_feat, gt_inst, instance_classes):
        instance_cls = gt_inst.gt_labels
        instance_box = gt_inst.gt_boxes
        instance_box_volume = gt_inst.instance_box_volume
        ...
        ps_inst = self.get_ps_instance3D(ps_instance_label, ps_semantic_label, superpoint, scan_id, coord_float)
        return ps_inst, ps_prob_label, one_box_points_preds, one_box_points_labs