...
def get_coords_color(opt):
    ...
    file = osp.join('./datasets/scannetv2/val', opt.room_name + '_inst_nostuff.pth')
    xyz, rgb, superpoint, _, _ = torch.load(file)
    rgb = (rgb + 1) * 127.5
    ...
    # same color order according to instance pointnum
        ...
        pass
    if inst_label.any()==36:
        print(opt.room_name)
    sem_valid = (inst_label != -100)
    xyz = xyz[sem_valid]
    rgb = rgb[sem_valid]
...
def get_coords_color_sem(opt):
    ...
    # same color order according to instance pointnum
        ...
        for i_ in range(len(masks) - 1, -1, -1):
            ...
            ins_pointnum[i] = mask.sum()
            inst_label[mask == 1] = i
            #print('masks[i][1]', masks[i][1])
            sem_color[i] = masks[i][1]
        sort_idx = np.argsort(ins_pointnum)[::-1]
        for _sort_id in range(ins_num):
            inst_label_pred_rgb[inst_label == sort_idx[_sort_id]] = COLOR_DETECTRON2[int(sem_color[sort_idx[_sort_id]]) % len(COLOR_DETECTRON2)]
        rgb = inst_label_pred_rgb
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prediction_path', help='path to the prediction results', default='/')
    parser.add_argument('--room_name', help='room_name', default='scene0355_01')
    ...
    if not os.path.exists(opt.out):
        os.mkdir(opt.out)
    '''
    opt.room_name = opt.room_name
    out = os.path.join(opt.out, opt.room_name + '.ply')
    print(opt.room_name)
    print(out)
    ...
    if opt.out:
        assert '.ply' in out, 'output cloud file should be in FILE.ply format'
        write_ply(points, colors, None, out)
    else:
        ...
        vis.destroy_window()
    '''
    room_names = os.path.join(opt.prediction_path, 'pred_instance')
    datanames = os.listdir(room_names)
    for dataname in datanames:
        if os.path.splitext(dataname)[1] == '.txt':
            opt.room_name = dataname[:-4]
            out = os.path.join(opt.out, dataname[:-4] + '.ply')
            print(opt.room_name)
            print(out)
