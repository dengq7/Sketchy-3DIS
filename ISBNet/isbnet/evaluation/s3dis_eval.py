...
class S3DISEval(object):
    ...
    def single_process(self, preds, gt_sem, gt_ins):
        ...
        pred_sem = np.zeros(gt_sem.shape[0], dtype=np.int32)
        pred_ins = np.zeros(gt_sem.shape[0], dtype=np.int32)
    ...
    def evaluate(self, pred_list, gt_sem_list, gt_ins_list):
        ...
        for i_sem in range(self.num_classes):
            tp = np.asarray(self.tpsins[i_sem]).astype(np.float32)
            fp = np.asarray(self.fpsins[i_sem]).astype(np.float32)
            tp = np.sum(tp)
            fp = np.sum(fp)
