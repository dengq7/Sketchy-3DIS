...
class S3DISDataset(CustomDataset):
    ...
    def load(self, filename):
        ...
        if self.with_normal:
            normal_filename = osp.join(self.data_root, "normals", scan_id + ".pth")
            normal = torch.load(normal_filename)
        else:
            normal=None
        ...
        return xyz, rgb, semantic_label, instance_label, spp, normal
    ...
    def transform_test(self, xyz, rgb, semantic_label, instance_label, spp, normal=None):
        ...
        for batch, piece in enumerate([piece_1, piece_2, piece_3, piece_4]):
            ...
            if normal is not None:
                normal_list.append(normal[piece])
        ...
        if normal is not None:
            normal = np.concatenate(normal_list, 0)
