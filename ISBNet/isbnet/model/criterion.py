...
class Criterion(nn.Module):
    ...
    def cal_point_wise_loss(
        ...
        if len(one_box_points_preds) == 0:
            ps_cls_loss = torch.tensor([0.0], device=cls_logits.device)
        else:
            ps_cls_loss = F.cross_entropy(one_box_points_preds, one_box_points_labs.long())
        losses['ps_cls_loss'] = ps_cls_loss
    ...
    def single_layer_loss(
        ...
        for b in range(batch_size):
            ...
            box_scale_preds_b = box_scale_preds[b]

            prob_labels_b = ps_prob_label[b_s:b_e]
            ...
            gt_bbox_scale = 0.5 * torch.exp(gt_iou - 1.0) + 0.5
            
            box_scale_pred = box_scale_preds_b[pred_inds]
            loss_dict["bbox_scale_loss"] = (
                loss_dict["bbox_scale_loss"] + F.mse_loss(box_scale_pred, gt_bbox_scale, reduction="sum") / num_gt_batch
            )
            ...
            if len(one_box_points_preds) == 0:
                ...
                ps_cls_loss = F.cross_entropy(one_box_points_preds, one_box_points_labs.long())
            loss_dict['ps_cls_loss'] =  loss_dict["ps_cls_loss"] + ps_cls_loss
    ...
    def forward(self, batch_inputs, model_outputs):
        ...
        if self.semantic_only or self.trainall:
            ...
            ps_prob_label = model_outputs["prob_label"]
            one_box_points_preds = model_outputs["one_box_points_preds"]
            one_box_points_labs = model_outputs["one_box_points_labs"]
        ...
        for k in self.loss_weight:
            ...
            loss_dict["aux01_" + k] = torch.tensor(
                0.0, requires_grad=True, device=semantic_labels.device, dtype=torch.float
            )
        ...
        box_scale_logits = model_outputs["box_scale_logits"]
        ...
        ps_prob_label = model_outputs['ps_prob_label']
        ...
        cls_logits_aux = model_outputs["cls_logits_aux"]
        mask_logits_aux = model_outputs["mask_logits_aux"]
        conf_logits_aux = model_outputs["conf_logits_aux"]
        box_preds_aux = model_outputs["box_preds_aux"]
        box_scale_logits_aux = model_outputs["box_scale_logits_aux"]
        
        aux_main_loss_dict_01 = self.single_layer_loss(
            cls_logits_aux,
            mask_logits_aux,
            conf_logits_aux,
            box_preds_aux,
            box_scale_logits_aux,
            ps_prob_label,
            dc_batch_offsets,
            one_box_points_preds,
            one_box_points_labs,
        ...
        coef_aux_01 = 0.5
        for k, v in self.loss_weight.items():
            loss_dict["aux01_" + k] = loss_dict["aux01_" + k] + aux_main_loss_dict_01[k] * v * coef_aux_01
