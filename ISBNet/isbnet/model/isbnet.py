...
class ISBNet(nn.Module):
    def __init__(
        ...
        self.ps_labeler = PseudoLabeler(d_model=channels)
        ...
        if not self.semantic_only:
            ...
            self.inst_box_scale_head = GenericMLP(
                input_dim=instance_head_cfg.dec_dim,
                hidden_dims=[instance_head_cfg.dec_dim, instance_head_cfg.dec_dim],
                norm_fn_name="bn1d",
                activation="relu",
            ...
            self.voxel_feats_head = nn.Sequential(
                nn.Linear(self.channels, self.channels), 
                nn.ReLU(), 
                nn.Linear(self.channels, self.channels)
            )
            '''
            self.voxel_feats_mead_head = nn.Sequential(
                nn.Linear(self.channels*2, self.channels), 
                nn.ReLU(), 
                nn.Linear(self.channels, self.channels)
            )

            self.voxel_feats_prop_head = nn.Sequential(
                nn.Linear(self.channels*2, self.channels), 
                nn.ReLU(), 
                nn.Linear(self.channels, self.channels)
            )
    ...
    def forward_train(
        ...
        **kwargs
        ...
        # NOTE voxelization
        gt_voxel_semantic_labels = semantic_labels[p2v_map[:, 1].long()]
        gt_voxel_instance_labels = instance_labels[p2v_map[:, 1].long()]
        gt_voxel_instance_labels = get_cropped_instance_label(gt_voxel_instance_labels)
        ...
        gt_instance_cls, gt_instance_box, gt_centroid_offset_labels, gt_corners_offset_labels, gt_instance_box_volume = get_instance_info(
            voxel_coords_float, gt_voxel_instance_labels, gt_voxel_semantic_labels, label_shift=self.label_shift
        )
        ...
        sp_coords = custom_scatter_mean(coords_float, spps, dim=0, pool=self.use_spp_pool)
        sp_feats = custom_scatter_mean(voxel_output_feats[v2p_map.long()], spps, dim=0, pool=self.use_spp_pool)
        spp_ids = custom_scatter_mean(spps, spps, dim=0, pool=self.use_spp_pool)

        voxel_semantic_labels, voxel_instance_labels, prob_label, one_box_points_preds, one_box_points_labs = self.ps_labeler(
                sp_coords, sp_feats, spp_ids, gt_instance_cls, gt_instance_box, gt_instance_box_volume, self.instance_classes)
     
        voxel_semantic_labels = voxel_semantic_labels[spps]
        voxel_instance_labels = voxel_instance_labels[spps]
        #prob_label = prob_label[spps]

        voxel_semantic_labels = voxel_semantic_labels[p2v_map[:, 1].long()].long()
        voxel_instance_labels = voxel_instance_labels[p2v_map[:, 1].long()].long()
        #prob_label = prob_label[p2v_map[:, 1].long()]

        instance_cls, instance_box, centroid_offset_labels, corners_offset_labels = get_ps_instance_info(
            voxel_coords_float, voxel_instance_labels, voxel_semantic_labels, self.instance_classes
        )
        '''
        voxel_semantic_labels, voxel_instance_labels, prob_label, one_box_points_preds, one_box_points_labs = self.ps_labeler(
                voxel_coords_float, voxel_output_feats, voxel_spps, gt_instance_cls, gt_instance_box, gt_instance_box_volume, self.instance_classes)

        instance_cls, instance_box, centroid_offset_labels, corners_offset_labels = get_ps_instance_info(
            voxel_coords_float, voxel_instance_labels, voxel_semantic_labels
        )
        '''
        voxel_semantic_labels[voxel_semantic_labels != self.instance_classes] = voxel_semantic_labels[voxel_semantic_labels != self.instance_classes] + self.label_shift
        ...
        if self.semantic_only or self.trainall:
            ...
            model_outputs.update(
                dict(
                    ...
                    one_box_points_preds=one_box_points_preds,
                    one_box_points_labs=one_box_points_labs,
                )
            )
        ...
        cls_logits, mask_logits, conf_logits, box_preds, box_scale_logits = self.forward_head(
            query_feats, query_locs, dc_mask_features, dc_coords_float, dc_box_preds, dc_batch_offsets
        )
        
        # -------------------------------
        
        medium_voxel_output_feats_ = get_medium_feats(box_preds, voxel_coords_float_, voxel_output_feats_, mask_logits, dc_batch_offsets)
        prop_voxel_output_feats_ = get_proposal_feats(box_preds, voxel_coords_float_, voxel_output_feats_, box_scale_logits, dc_batch_offsets)
        
        voxel_output_feats_aux_ = voxel_output_feats_ + medium_voxel_output_feats_ + prop_voxel_output_feats_
        #voxel_output_feats_aux_ = torch.cat((voxel_output_feats_, medium_voxel_output_feats_, prop_voxel_output_feats_), dim=-1)
        voxel_output_feats_aux_ = self.voxel_feats_head(voxel_output_feats_aux_)
        query_locs_aux, query_feats_aux, query_boxes_aux, query_inds1_aux = self.point_aggregator1(
            voxel_coords_float_,
            voxel_output_feats_aux_,
            voxel_box_preds_,
            batch_offsets=voxel_batch_offsets_,
            batch_size=batch_size,
            sampled_before=False,
        ...
        query_locs_aux, query_feats_aux, query_boxes_aux, query_inds_aux = self.point_aggregator2(
            query_locs_aux, query_feats_aux, query_boxes_aux, batch_offsets=None, batch_size=batch_size, sampled_before=True
        )

        cls_logits_aux, mask_logits_aux, conf_logits_aux, box_preds_aux, box_scale_logits_aux = self.forward_head(
            query_feats_aux, query_locs_aux, dc_mask_features, dc_coords_float, dc_box_preds, dc_batch_offsets
        )
        ...
        model_outputs.update(
            dict(
                ...
                ps_prob_label=prob_label,
                dc_batch_offsets=dc_batch_offsets,
                cls_logits=cls_logits,
                mask_logits=mask_logits,
                conf_logits=conf_logits,
                box_preds=box_preds,
                box_scale_logits=box_scale_logits,
                one_box_points_preds=one_box_points_preds,
                one_box_points_labs=one_box_points_labs,
                cls_logits_aux=cls_logits_aux,
                mask_logits_aux=mask_logits_aux,
                conf_logits_aux=conf_logits_aux,
                box_preds_aux=box_preds_aux,
                box_scale_logits_aux=box_scale_logits_aux,
            )
        )
    ...
    def forward_test(
        ...
        **kwargs
        ...
        if self.semantic_only:
            ...
            _, _, centroid_offset_labels, corners_offset_labels, _ = get_instance_info(
        ...
        for i in range(len(n_sample_arr)):
            ...
            cls_logits, mask_logits, conf_logits, box_preds, box_scale_logits = forward_head_outputs
    ...
    def forward_head(
        ...
        box_scale_logits = self.inst_box_scale_head(queries_features).transpose(1, 2).squeeze(-1)
        ...
        if inference:
            ...
            box_scale_logits = box_scale_logits[0, fg_conds].unsqueeze(0)
        ...
        if inference:
            return cls_logits[0], mask_logits[0], conf_logits[0], queries_box_preds[0], box_scale_logits[0]
        return cls_logits, mask_logits, conf_logits, queries_box_preds, box_scale_logits
