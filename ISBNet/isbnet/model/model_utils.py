...
def get_instance_info(coords_float, instance_labels, semantic_labels, label_shift=2):
    ...
    for i_ in range(instance_num):
        ...
        min_xyz_i = torch.from_numpy(np.matmul(min_xyz_i.cpu().numpy(), rot_matrix)).float().cuda()
        max_xyz_i = torch.from_numpy(np.matmul(max_xyz_i.cpu().numpy(), rot_matrix)).float().cuda()
        '''       
        '''
        ## S5:  scaled + translated + rotated
        max_xyz_i = max_xyz_i + 0.5 * coord_extent
        axis = ((max_xyz_i + min_xyz_i)/2).cpu().numpy()
        radian = 5 * math.pi/180
        rot_matrix = scipy.linalg.expm(np.cross(np.eye(3), axis / scipy.linalg.norm(axis) * radian))
        ...
        instance_box_volume.append(torch.prod(torch.clip((max_xyz_i - min_xyz_i), min=0.0, max=None)))
    ...
    instance_box_volume = torch.stack(instance_box_volume, dim=0)  # N, 6
    return instance_cls, instance_box, centroid_offset_labels, corners_offset_labels, instance_box_volume


def get_ps_instance_info(coords_float, instance_labels, semantic_labels, instance_classes, label_shift=2):
    instance_pointnum = []
    instance_cls = []
    instance_box = []
    ...
    ps_ins_box = (
        torch.ones(6, dtype=coords_float.dtype, device=coords_float.device) * -100.0
    )
    ...
    for i_ in range(instance_num):
        ...
        if len(inst_idx_i)==0:
            instance_box.append(ps_ins_box)
            instance_cls.append(torch.tensor([instance_classes]).to(instance_labels.device))
            continue
...
def get_spp_gt(
    ...
    for b in range(batch_size):
        ...
        #print('instance_cls:', instance_cls.shape)
