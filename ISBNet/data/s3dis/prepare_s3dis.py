...
parser.add_argument(
    "--data_dir", type=str, default="./datasets/s3dis/Stanford3dDataset_v1.2_Aligned_Version/", help="Path to the original data"
)
...
def read_scene_txt(name, data_dir):
    area = name.split(".")[0]
    name = name.split(".")[1]
    print('data_path:', os.path.join(data_dir + "/" + area, name, name + ".txt"))
...
def preprocess_s3dis(data_dir):
    ...
    for scene_name in scene_list:
        area = scene_name.split(".")[0]
        name = scene_name.split(".")[1]
        save_dir = "./datasets/s3dis/preprocess"
